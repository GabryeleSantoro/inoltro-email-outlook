...
def test_nome_con_data_e_ora(tmp_path: Path) -> None:
    path = session_log_path(tmp_path / "inoltro.log", datetime(2025, 5, 21, 9, 15, 0))
    assert path == tmp_path / "inoltro-20250521-091500.log"
...
def test_nome_progressivo_se_il_file_esiste(tmp_path: Path) -> None:
    ...
    (tmp_path / "inoltro-20250521-091500.log").write_text("", encoding="utf-8")

    assert session_log_path(tmp_path / "inoltro.log", momento).name == "inoltro-20250521-091500-1.log"


def test_ogni_sessione_scrive_sul_proprio_file(tmp_path: Path) -> None:
    base = tmp_path / "logs" / "inoltro.log"
    ...
    assert primo == tmp_path / "logs" / "inoltro-20250521-091500.log"
    assert secondo == tmp_path / "logs" / "inoltro-20250521-103000.log"
...
def test_pulizia_conserva_i_piu_recenti(tmp_path: Path) -> None:
    base = tmp_path / "inoltro.log"
    for ora in range(5):
        (tmp_path / f"inoltro-20250521-0{ora}0000.log").write_text("", encoding="utf-8")

    rimossi = prune_session_logs(base, keep=2)

    assert [p.name for p in rimossi] == [
        "inoltro-20250521-000000.log",
        "inoltro-20250521-010000.log",
        "inoltro-20250521-020000.log",
    ]
    assert sorted(p.name for p in tmp_path.glob("*.log")) == [
        "inoltro-20250521-030000.log",
        "inoltro-20250521-040000.log",
    ]
...
def test_pulizia_non_tocca_altri_file(tmp_path: Path) -> None:
    ...
    (tmp_path / "inoltro-vecchio.log").write_text("", encoding="utf-8")
    (tmp_path / "inoltro-20250521-000000.log").write_text("", encoding="utf-8")
...
def test_setup_logging_pulisce_i_vecchi(tmp_path: Path) -> None:
    ...
    base = tmp_path / "inoltro.log"
    (tmp_path / "inoltro-20250101-000000.log").write_text("", encoding="utf-8")
    (tmp_path / "inoltro-20250102-000000.log").write_text("", encoding="utf-8")
    ...
    assert sorted(p.name for p in tmp_path.glob("*.log")) == [
        "inoltro-20250102-000000.log",
        "inoltro-20250521-091500.log",
    ]
