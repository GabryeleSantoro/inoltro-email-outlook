...
def ambiente_pulito(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for name in ("SERVICE_API_KEY", "API_HOST", "PORT"):
...
def test_ocr_locale_non_richiede_chiave(tmp_path: Path) -> None:
    assert Settings.load(write_config(tmp_path, VALID_YAML)).ocr.device == "cpu"
...
def test_device_ocr_non_valido(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    yaml_text = VALID_YAML.replace("device: cpu", "device: gpu")
